...
This repository follows a documentation-first coding standard that keeps code easy to review and maintain. The guidance lives in the [`.github/instructions`](.github/instructions) folder and is intentionally opinionated about when comments are useful.

- Comment the intent, trade-off, or non-obvious decision — not the mechanics that the code already makes obvious.
- Remove stale or redundant comments in the same change that updates the related logic.
- Document exported helpers in `db/` and `src/lib/` with TSDoc/JSDoc describing the purpose, parameters, and return value, including the injectable `db` argument used by tests.
- Document reusable `.astro` component `Props` interfaces so the component contract is explicit.
- Keep TypeScript precise and readable: prefer explicit return types and type-only imports, and let ESLint enforce the import conventions where practical.
